# Knowledge Tracker

Living record of what Tarek has learned, with confidence levels.

## Confidence legend

| Symbol | Meaning |
|--------|---------|
| ⏳ | Not yet covered |
| 🔴 | Exposed — encountered but not internalized |
| 🟡 | Comfortable — understands but not yet automatic |
| ✅ | Solid — can teach it back unprompted |

A concept is **"solid" only after Tarek has explained it back in his own words**
(without looking at the source material) and predicted at least one consequence
of it correctly.

---

## Phase 1 — Chemistry fundamentals

### Matter & atoms
- ⏳ What "matter" is, and its relationship to mass and energy
- ⏳ What an atom is and how we know atoms exist
- ⏳ Scale of atoms (size, why we can't see them)

### Atomic structure
- ⏳ The nucleus (protons + neutrons)
- ⏳ Electrons and electron shells / orbitals
- ⏳ Why atoms are mostly empty space
- ⏳ Forces inside the atom (strong nuclear, electromagnetic)

### Electric charge
- ⏳ What "charge" is (positive / negative / neutral)
- ⏳ Coulomb's law in plain language
- ⏳ Why opposite charges attract and like charges repel
- ⏳ Charge as a conserved quantity

### Elements & the periodic table
- ⏳ What an element is
- ⏳ Why proton count = element identity
- ⏳ How the periodic table is organized (period, group, block)
- ⏳ How elements were discovered, why we keep finding new ones
- ⏳ Trends across the periodic table (electronegativity, atomic radius, etc.)

### Atomic mass / weight
- ⏳ Atomic mass unit (amu / Da)
- ⏳ Distinction: atomic mass vs. atomic weight vs. mass number
- ⏳ Why most elements show non-integer atomic weights

### Isotopes
- ⏳ Definition (same Z, different N)
- ⏳ Why isotopes exist
- ⏳ Stable vs. radioactive isotopes
- ⏳ Why isotopes matter (dating, tracing, nuclear fuel, medicine)
- ⏳ Lithium-6 vs. Lithium-7 specifically

### Ions & ionization
- ⏳ What an ion is (cation, anion)
- ⏳ How ions form (loss / gain of electrons)
- ⏳ Why ions exist in nature (energetics, solvation)
- ⏳ Why we isolate ions (and how)
- ⏳ Ionic conductivity, mobility

### Chemical bonding
- ⏳ Ionic vs. covalent vs. metallic bonds
- ⏳ Why bonds form (energy minimization)
- ⏳ Hydrogen bonds, van der Waals (just enough for solutions)

### States of matter & solutions
- ⏳ Solid / liquid / gas / plasma
- ⏳ What "dissolved" actually means at the molecular level
- ⏳ Solvation shells
- ⏳ pH, acids, bases (lightweight)

### Lithium specifically
- ⏳ Lithium's position on the periodic table
- ⏳ Lithium's chemistry (alkali metal, lone valence electron)
- ⏳ Where lithium is found in nature
- ⏳ Industrial relevance of lithium

---

## Phase 2 — Water treatment

### General
- ⏳ What water treatment is and why we do it
- ⏳ Drinking water vs. wastewater vs. industrial
- ⏳ Coagulation / flocculation / sedimentation
- ⏳ Filtration (sand, carbon, multimedia)
- ⏳ What water treatment is and why we do it
- ⏳ Disinfection (chlorine, UV, ozone)
- ⏳ Ion exchange resins
- ⏳ Membrane processes (MF / UF / NF / RO)
- ⏳ Electrodialysis & capacitive deionization
- ⏳ Drinking water vs. wastewater vs. industrial
- ⏳ Distillation
- ⏳ Why water-treatment unit ops are the right vocabulary for ion separation
- ⏳ Coagulation / flocculation / sedimentation
- ⏳ Filtration (sand, carbon, multimedia)
- ⏳ Disinfection (chlorine, UV, ozone)
- ⏳ Ion exchange resins
- ⏳ Membrane processes (MF / UF / NF / RO)
- ⏳ Electrodialysis & capacitive deionization
- ⏳ Distillation
- ⏳ Why water-treatment unit ops are the right vocabulary for ion separation

---

## Phase 3 — Lithium isotope separation

### General
- ⏳ Why separate Li-6 from Li-7 (fusion, fission, scientific)
- ⏳ The natural abundance ratio (~7.5% / 92.5%)
- ⏳ Mass-difference physics & why ~17% is "a lot" but still hard
- ⏳ Separation factor (α) — what it means and typical values
- ⏳ COLEX (mercury amalgam) — chemistry, history, environmental issues
- ⏳ Electromagnetic separation (calutron)
- ⏳ Laser methods (AVLIS, MLIS)
- ⏳ Distillation
- ⏳ Chemical exchange without mercury
- ⏳ Ion-exchange chromatography
- ⏳ Membrane separation
- ⏳ Electrochemical separation
- ⏳ MOFs / crown ethers / ionic liquids
- ⏳ Industrial / geopolitical context (who supplies Li-6, fusion implications)
- ⏳ Why separate Li-6 from Li-7 (fusion, fission, scientific)
- ⏳ The natural abundance ratio (~7.5% / 92.5%)
- ⏳ Mass-difference physics & why ~17% is "a lot" but still hard
- ⏳ Separation factor (α) — what it means and typical values
- ⏳ COLEX (mercury amalgam) — chemistry, history, environmental issues
- ⏳ Electromagnetic separation (calutron)
- ⏳ Laser methods (AVLIS, MLIS)
- ⏳ Distillation
- ⏳ Chemical exchange without mercury
- ⏳ Ion-exchange chromatography
- ⏳ Membrane separation
- ⏳ Electrochemical separation
- ⏳ MOFs / crown ethers / ionic liquids
- ⏳ Industrial / geopolitical context (who supplies Li-6, fusion implications)

---

## Session log

(Each teaching session adds a brief entry: date, what we covered, what stuck,
what didn't, and the new confidence symbols promoted.)

| Date | Covered | Promoted | Notes |
|------|---------|----------|-------|
| _(no sessions yet)_ | | | |
